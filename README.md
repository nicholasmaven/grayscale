...
尽管大多数情况下公司/团队都有全链路灰度服务(一般做在入口网关层), 但以下场景不适用全链路灰度
1. 异步调用: 例如ekyc调ds的api基本都是异步(除了OCR是同步执行)
2. 不具备全链路灰度条件(例如ekyc的灰度只有一台), 又或者不需要全链路灰度
...
组件不支持全链路灰度, 全链路灰度这需要上下游链路对等灰度部署, 一般通过流量染色的方式实现
...
### 组件依赖
apollo配置中心, 因本人团队实际在用apollo的缘故, 其实换成其他的配置中心也可以
...
对照模式的限制: 对照和常规逻辑的出入参类型一致

## 主要思考
1、为什么要限制一个bizType只有一个灰度属性?
实际的业务灰度中, 绝大多数情况都是单维度的灰度
...
5、为什么要提供对照功能?
接口升级切换灰度时需要对照新旧接口返回, 增强灰度的结果验证能力
重构代码后, 新旧方法块/逻辑级别很难通过简单的单测来覆盖验证, 对照有助于这一过程
